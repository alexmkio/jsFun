...
const clubPrompts = {
  membersBelongingToClubs() {
    ...
    //   ...etc
    // }
    //use reduce method, return obj
    //keys: member's name values: [clubs name]
    const result = clubs.reduce((obj, group) => {
      group.members.forEach(member => {
        if (!obj[member]) {
          obj[member] = [];
        }
        obj[member].push(group.club);
      });
      return obj;
    }, {});
    return result;
...
const bookPrompts = {
    ...
    const weatherPrompts = {
      getAverageTemps() {
        ...
        // toget avg = temp's high + low/ 2
        // arr > obj > temp(obj) has two keys ...
        // map
        const result = weather.map(elem => (elem.temperature.high + elem.temperature.low) / 2);
        return result;
      ...
      findSunnySpots() {
        ...
        //filter type = sunny
        // map location
        const result = weather.filter(elem => elem.type.includes('sunny')).map(loc => loc.location + ` is ${loc.type}.`);
        return result;
    ...
    const nationalParksPrompts = {
      ...
      getParkActivities() {
        ...
        //   'backpacking',
        //   'rock climbing' ]
        return nationalParks.reduce((arr, park) => {
      ...
      getParkInEachState() {
        ...
        //result: [ obj elements]
        //reduce
        //keys: park.location values : park.name
        //
    ...
    const breweryPrompts = {
      getBeerCount() {
        ...
        return breweries.reduce((sum, brew) => {
      ...
      getBreweryBeerCount() {
        ...
        // ]
        // map > return {}
        //name: brew.name, beerCount: brew.beers.len
      ...
      findHighestAbvBeer() {
        ...
        let result = breweries
          .map(brew => brew.beers.sort((a, b) => b.abv - a.abv)[0])
    ...
    const turingPrompts = {
      studentsForEachInstructor() {
        ...
        return instructors.map(teacher => {
          return {
            name: teacher.name,
            studentCount: cohorts.reduce((num, group) => {
              if (teacher.module === group.module) {
                num += group.studentCount;
              }
              return num;
            }, 0)
          }
      ...
      studentsPerInstructor() {
        ...
        // reduce and return {}
        //`cohort${group.cohort}`
        let num = instructors.reduce((obj, teacher) => {
          if (!obj[teacher.module]) {
            obj[teacher.module] = 0;
          }
          obj[teacher.module]++;
          return obj;
        }, {})
        // console.log(num);
        let test = cohorts.reduce((obj, group) => {
          obj[`cohort${group.cohort}`] = group.studentCount / num[group.module]
      ...
      modulesPerTeacher() {
        ...
        //     Will: [1, 2, 3, 4]
        //   }
        // return obj with reduce

        let test = instructors.reduce((obj, teacher) => {
          obj[teacher.name] = [];
          teacher.teaches.forEach(skill => {
            cohorts.forEach(group => {
              if (group.curriculum.includes(skill) &&
                !obj[teacher.name].includes(group.module)) {
                obj[teacher.name].push(group.module)
              }
            })
          })
          return obj;
      ...
      curriculumPerTeacher() {
        ...
        let test = cohorts.reduce((obj, group) => {
          group.curriculum.forEach(skill => {
            if (!obj[skill]) {
              obj[skill] = [];
            }
            instructors.forEach(teacher => {
              if (teacher.teaches.includes(skill) &&
                !obj[skill].includes(teacher.name)) {
                obj[skill].push(teacher.name)
              }
            })
          })
    ...
    const bossPrompts = {
      bossLoyalty() {
        ...
        return Object.values(bosses).map(leader => {
          return {
    ...
    const astronomyPrompts = {
      starsInConstellations() {
        ...
        return stars.reduce((arr, star) => {
          Object.values(constellations).forEach(key => {
            if (key.stars.includes(star.name)) {
              arr.push(star);
            }
          });
          return arr;
      ...
      starsByColor() {
        ...
        // return obj (reduce). keys: star.color  values: [star]
        let test = stars.reduce((obj, star) => {
          if (!obj[star.color]) {
            obj[star.color] = [];
          }
      ...
      constellationsStarsExistIn() {
        ...
        let test = stars.reduce((arr, star) => {
    ...
    const ultimaPrompts = {
      totalDamage() {
        ...
        // Answer => 113
        let test = characters.reduce((sum, char) => {
      ...
      charactersByTotal() {
        ...
        //return arr map
        // obj elems >> key: char.name   values:
        let test = characters.map(char => {
          let finalObj = {
            [char.name]: {
              damage: 0,
              range: 0
          ...
          char.weapons.forEach(weapon => {
            finalObj[char.name].damage += weapons[weapon].damage
            finalObj[char.name].range += weapons[weapon].range
          })
          return finalObj;
        })
